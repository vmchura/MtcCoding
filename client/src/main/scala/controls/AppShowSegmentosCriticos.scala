...
object AppShowSegmentosCriticos {
  ...
  def plotKmPeligrosos(data: Seq[(Int, Int)]): Unit = {
    val x = (1 to data.length).map(x => x * 100)
    val promedio = data.map(_._1.toDouble)
    val base = data.map(_._2.toDouble)
    println(data.mkString("\n"))
    plotlyutils.drawMap(x.toJSArray, promedio.toJSArray, base.toJSArray, "perfilCarretera")

  }

  def actionWhenRutaSelected(rutaSHMeta: RutaSHMeta): Unit = {
    rutaSelected.value = Some(rutaSHMeta)
    val playCall = JavaScriptRoutes.controllers.ReportController.getSegmentosVelocidad(rutaSHMeta.idRuta)

    //TODO
    val playAjax = new PlayAjax(playCall)
    val futResponse = playAjax.callByAjaxWithParser(dyn => read[Option[(Seq[(Int, Int)], Seq[LineStringSH], LineStringSH)]](dyn.response.toString))
  ...
  def updateReport(futResponse: Future[Either[String, Option[(Seq[(Int, Int)], Seq[LineStringSH], LineStringSH)]]]) = {
    futResponse.onComplete {
      ...
      case Success(Right(Some((promedioBase, criticos, lineString)))) =>
        println("SUCCESS!!")

        plotKmPeligrosos(promedioBase)

        val mapOL = mapOLOpt.getOrElse(new OLMapManager("mapShowExactRoad"))
        mapOL.cleanLayers()
        mapOL.addRoute(lineString)
        println(s"criticos: ${criticos.length}")
        criticos.foreach { mapOL.addRoute }
